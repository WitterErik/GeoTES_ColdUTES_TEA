...
op_hour1 = [6:8 18:21]; % List of hours of operation
op_hour2 = [6:8 18:21]; % List of hours of operation
op_hour3 = [6:8 18:21]; % List of hours of operation
...
gTES_flowrate.P = 60 ; % Flow rate per production well, L/s
gTES_flowrate.I = 60 ; % Flow rate per injection well, L/s
...
gTES_props.thickness = 200; % Production thickness
gTES_fluid = 'water';
...
finance.ITC         = 0.3 ;         % Investment tax credit
...
gTES.charge_production.p = 20 ; % Pressure of produced fluid during charge, bar
...
for i = 2:nY*8760
    ...
    % Otherwise all heat goes straight in the ground
    else
        % Send remaining heat to geo-TES if the wells have capacity
        dP = CSP.power(i) ;
...
results = [CSP.solar_multiple;CSP.available_solar_heat;...
    ...
    econ.LCOH_ITC;cap_payment_5;cap_payment_10;cap_hours;Ccap';LCOE_mat'];
