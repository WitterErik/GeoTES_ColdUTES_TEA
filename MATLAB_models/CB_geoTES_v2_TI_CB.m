...
% System level inputs
location = 'Elk Hills CA' ; % Choose from [Imperial CA], [Elk Hills CA], [ERCOT West]
...
HE_design.eff   = 0.178 ; % Turbine inlet temperature, C. Calculate Qin from this.
HE_design.TIT   = -1; % This value is unused
HE_design.Qin   = -1; % This value is unused
HE_design.fan   = 54.3 / 1000.0 ; % Air fan parasitic, fan power per net power output from heat engine (turbine work - compressor work)*generator_eff
...
HP_design.Win   = 15.5;% Power output, MW-e
HP_design.T0    = 25; % Design ambient temperature, C
HP_design.COP   = 3.8 ; % Coefficient of Performance. Calculate Qout from this.
...
%% Carnot Battery
CB.Tmax = 180 ; % Hot storage temperature, C
CB.Tmin = 55 ;   % Cold storage temperature, C
CB.RTeff = HE.eff0 * HP.COP0 ; % Round-trip efficiency, %

%% Cold thermal storage (geoTES) inputs. During charge, heat is extracted from the geo-fluids and it is pumped back into the ground, but not recovered during discharge.
gCTES_recovery = 0.95 ; % Fraction of input thermal energy that is recovered
gCTES_Tinit = 90 ; % Initial reservoir temperature, C
gCTES_flowrate.P = 60 ; % Flow rate per production well, L/s
gCTES_flowrate.I = 60 ; % Flow rate per injection well, L/s
...
gCTES_props.void = 0.32 ; % Porosity
gCTES_props.depth = 500 ; % Well depth, m
gCTES_props.thickness = 100 ;
gCTES_fluid = 'water';
...
gHTES_Tinit = 90 ; % Initial reservoir temperature, C
gHTES_flowrate.P = 60 ; % Flow rate per production well, L/s
gHTES_flowrate.I = 60 ; % Flow rate per injection well, L/s
...
gHTES_props.void = 0.32 ; % Porosity
gHTES_props.depth = 500 ; % Well depth, m
gHTES_props.thickness = 100 ;
gHTES_fluid = 'water';
...
gHTES.charge_production.p = 20 ; % Pressure of produced fluid during charge, bar
...
gHTES.discharge_injection.p = 20 ;
...
for i = 2:nY*8760
    ...
    elseif elec_prices(i) > econ.discharge_price
        ...
        % the hot storage
        % Cold storage is not discharged in this system
...
Cold_source_Drilling_Cost = DrillingCostUpdated(Ncold_source_well,0,gCTES.depth,200,"Small",1,1.5,"Deviated","Liner");
...
Cold_source_ProdPumping_Estimation = ProductionPumpingCost(ResTemp,Pflowrate,gCTES.charge_prod_Nwell,6370,gCTES.depth,200,"Small","Lineshaft","Liner",1.553);
...
% Drilling cost and completions
Cold_sink_Drilling_Cost = DrillingCostUpdated(Ncold_sink_well,0,gCTES.depth,200,"Small",1,1.5,"Deviated","Liner");
...
Cold_sink_InjPumping_Estimation = InjectionPumpingCost(ResTemp,InjTemp,Iflowrate,Pflowrate,PressureThermalSource,Ncold_sink_well,7645,gCTES.depth,200,"Small","Liner",1.533,"Charge");
...
% Pump Maintenance
Cold_sink_PumpMaintenance = 0;%Cold_sink_ProdPumping_Estimation.PumpMaintenanceCost(Step3,Step6);
...
% Drilling cost and completions
Hot_source_Drilling_Cost = DrillingCostUpdated(Nhot_source_well,0,gHTES.depth,200,"Small",1,1.5,"Deviated","Liner");
...
Hot_source_ProdPumping_Estimation = ProductionPumpingCost(ResTemp,Pflowrate,gHTES.charge_prod_Nwell,6370,gHTES.depth,200,"Small","Lineshaft","Liner",1.553);
...
Hot_source_InjPumping_Estimation = InjectionPumpingCost(ResTemp,InjTemp,Iflowrate,Pflowrate,PressureThermalSource,gHTES.discharge_inj_Nwell,7645,gHTES.depth,200,"Small","Liner",1.533,"Charge");
...
% Drilling cost and completions
Hot_sink_Drilling_Cost = DrillingCostUpdated(Nhot_sink_well,0,gHTES.depth,200,"Small",1,1.5,"Deviated","Liner");
...
Hot_sink_ProdPumping_Estimation = ProductionPumpingCost(ResTemp,Pflowrate,gHTES.discharge_prod_Nwell,6370,gHTES.depth,200,"Small","Lineshaft","Liner",1.553);
...
Hot_sink_InjPumping_Estimation = InjectionPumpingCost(ResTemp,InjTemp,Iflowrate,Pflowrate,PressureThermalSource,gHTES.charge_inj_Nwell,7645,gHTES.depth,200,"Small","Liner",1.533,"Charge");
...
cold_discharge_production_pump = 0;%discharging_hours * Cold_sink_ProdPumping_Cost_Duty(1) / 1e6 ;
cold_discharge_injection_pump = 0;%discharging_hours * Cold_source_InjPumping_Cost_Duty(1) / 1e6 ;
...
Ccap = [Total_Exploration_Cost FieldDevPlantPermitting  ...
    ...
    Hot_sink_InjPumping_Cost_Duty(2) Hot_sink_FlowLineCost ...
    Total_Cold_source_Drilling_Cost Cold_source_ProdPumping_Cost_Duty(2) ...
    0 Cold_source_FlowLineCost ...
    Total_Cold_sink_Drilling_Cost 0 ...
    Cold_sink_InjPumping_Cost_Duty(2) Cold_sink_FlowLineCost ...
