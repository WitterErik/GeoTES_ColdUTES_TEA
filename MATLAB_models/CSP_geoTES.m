...
solar_multiple = 7.1;%5;%7.1;%6.3 %6;
...
HTPC_type = 'HE'; % Type of power cycle [heat engine]
HTPC_design.Wout  = 10 ;% Power output, MW-e
HTPC_design.T0    = 15; % Design ambient temperature, C
HTPC_design.TIT   = CSP.Tmax - 10 ; % Turbine inlet temperature, C. Calculate efficiency and Qin from this.

HTPC_foff = 'data\example_off_design.xlsx' ; % File location specifying off-design behaviour

HTPC_cost.power_block = 1000 ; % power block, $/kW-e
HTPC_cost.HX = 250 ; % heat exchanger between TES and power block, $/kWh-th
...
LTPC_design.Wout  = 10 ;% Power output, MW-e
...
TES_duration = 2;%6 ; % storage duration, hours
TES_fluid = 'nitrate salt' ;
TES_Thot = CSP.Tmax ; % Max. storage temperature, C
TES_Tcld = CSP.Tmin ; % Min. storage temperature, C
TES_Qloss = 1 ; % Tank heat leakage per day, %
TES_max_tank_vol = 50000 ; % m3
ins.k     = 0.08 ; % Thermal conductivity of insulation
ins.rho   = 150 ; % Density of insulation

TES_cost.fluid = 1 ; % Storage fluid cost, $/kg
TES_cost.tank = 0 ; % Use tank cost correlation number
TES_cost.insulation = 50 ; % Insulation cost, %/m3

% Set up high temperature TES class
TES = TES_class(TES_duration, HTPC.Qin0, TES_fluid, TES_Thot, TES_Tcld, TES_Qloss,TES_max_tank_vol, ins,TES_cost) ;
TES = tank_sizes(TES);
...
gTES_recovery = 0.85 ; % Fraction of input thermal energy that is recovered
gTES_Tinit = 50 ; % Initial reservoir temperature, C
gTES_flowrate = 60 ; % Flow rate per well, L/s
gTES_Qmax = 1000 ; % Maximum charging power input, MWh-th
...
out = call_SAM(CSP,HTPC) ; % This function needs to be incorporated to CSP class
...
HTPC.Tamb = [out.Tamb;out.Tamb] ;
LTPC.Tamb = HTPC.Tamb ;

HTPC.Qin(1) = CSP.power(1) ;
HTPC = interpolate_off_design(HTPC,1);
...
for i = 2:2*8760
    ...
    % If solar heat exceeds design power input to high-temp power cycle
    if CSP.power(i) > HTPC.Qin0
        % Then send design heat to power cycle and calculation power output
        HTPC.Qin(i) = HTPC.Qin0 ;
        HTPC = interpolate_off_design(HTPC,i);

        % Send remaining heat to hot TES if there is capacity
        ...
        if dTES > dP*dT
            ...
            % Remaining heat
            dP = dP - TES.power(i) ;
            % This goes into the subsurface if the wells have capacity
            % Otherwise it gets dumped
            % [Can consider other options where it goes to LTPC first, then
            % excess is dumped. Or run HTPC, then LTPC, then TES, then
            % gTES, or other variations which may depend on price signals. 
            % Could also run LTPC during summer when high ambient temperatures
            % reduce HTPC output].
            if dP < gTES.Qmax
                gTES.power(i) = dP ;
                %gTES.energy(i) = gTES.energy(i-1) + dP * dT ;
            else
    ...
    % If solar heat is less than design point then first dispatch high-temp
    % TES. Then dispatch geoTES through the LTPC if the high-temp TES
    % becomes depleted.
    else
        % Required heat
        ...
        if dP * dT < TES.energy(i-1) 
            TES.power(i) = -dP ;
            TES.energy(i) = TES.energy(i-1) + TES.power(i) ;
            HTPC.Qin(i) = HTPC.Qin0 ;
            HTPC = interpolate_off_design(HTPC,i);
        else
            ...
            % Loss in power delivered
            dW = HTPC.Wout0 - HTPC.Wout(i) ;
            % Estimate heat (on surface) from LTPC to replace this
            dQ = dW / LTPC.eff0 ;

            % If heat exceeds design heat input to LTPC, then dispatch the
            % design heat for LTCH (if gTES has sufficient capacity)
            if dQ >= LTPC.Qin0 
                if LTPC.Qin0 > gTES.energy(i-1)
                    gTES.power(i) = -gTES.energy(i-1)/dT;
                else
                    gTES.power(i) = -LTPC.Qin0;
                end
                
            else
                % Otherwise dispatch just the quantity of heat that is
                % required
...
% Annual results for the CSP, power cycles, and storage
CSP  = CSP_annual_energy(CSP);
HTPC = PC_annual_energy(HTPC);
LTPC = PC_annual_energy(LTPC);

gTES = geoTES_annual_energy(gTES) ;
TES = TES_annual_energy(TES) ;

% Calculate sub-system costs
CSP  = calc_CSP_cost(CSP,HTPC) ;
HTPC = calc_PC_cost(HTPC) ;
LTPC = calc_PC_cost(LTPC) ;
TES  = calc_TES_cost(TES) ;
...
Charge_Drilling_Cost = DrillingCost(gTES.charge_prod_Nwell,gTES.charge_inj_Nwell,2500,500,"Large",1,2.2,"Vertical","Openhole");
...
Charge_ProdPumping_Estimation = ProductionPumpingCost(ResTemp,Pflowrate,gTES.charge_prod_Nwell,2500,2500,500,"Large","Lineshaft","Openhole",1.553);
...
Charge_InjPumping_Estimation = InjectionPumpingCost(ResTemp,InjTemp,Iflowrate,Pflowrate,PressureThermalSource,gTES.charge_inj_Nwell,3000,2500,500,"Large","Openhole",1.533,"Discharge");
...
% Drilling cost and completions
Discharge_Drilling_Cost = DrillingCost(gTES.discharge_prod_Nwell,gTES.discharge_inj_Nwell,2500,500,"Large",1,2.2,"Vertical","Openhole");
...
Discharge_ProdPumping_Estimation = ProductionPumpingCost(ResTemp,Pflowrate,gTES.discharge_prod_Nwell,2500,2500,500,"Large","Lineshaft","Openhole",1.553);
...
Discharge_InjPumping_Estimation = InjectionPumpingCost(ResTemp,InjTemp,Iflowrate,Pflowrate,PressureThermalSource,gTES.discharge_inj_Nwell,3000,2500,500,"Large","Openhole",1.533,"Discharge");
...
econ.surface_capital_cost = CSP.total_cost + TES.total_cost + HTPC.total_cost + LTPC.total_cost;
...
fprintf(1,"High-temp power cycle power output        = %6.2f MW-e\n",HTPC.Wout0);
fprintf(1,"Average high-temp power cycle output      = %6.2f MW-e\n",mean(HTPC.Wout(HTPC.Wout(8761:2*8760)>0)));
fprintf(1,"Low-temp power cycle power output         = %6.2f MW-e\n",LTPC.Wout0);
fprintf(1,"Average low-temp power cycle output       = %6.2f MW-e\n",mean(LTPC.Wout(LTPC.Wout(8761:2*8760)>0)));
...
fprintf(1,"Heat delivered to high-temp. cycle        = %6.2f GWh-th\n",HTPC.Qin_tot);
fprintf(1,"Electricity generated by high-temp. cycle = %6.2f GWh-e\n\n",HTPC.Wout_tot);

fprintf(1,"Heat delivered to low-temp. cycle         = %6.2f GWh-th\n",LTPC.Qin_tot);
fprintf(1,"Electricity generated by low-temp. cycle  = %6.2f GWh-e\n\n",LTPC.Wout_tot);

fprintf(1,"Total electricity generateion             = %6.2f GWh-e\n\n",HTPC.Wout_tot+LTPC.Wout_tot);
...
fprintf(1,'High-temp. thermal storage cost = %6.2f M$\n',TES.total_cost/1e6)
fprintf(1,'High-temp. power cycle cost     = %6.2f M$\n',HTPC.total_cost/1e6)
...
Ccap = [Total_Exploration_Cost FieldDevPlantPermitting  ...
    ...
    CSP.total_cost TES.total_cost HTPC.total_cost LTPC.total_cost]/1e6;
xlab = {'Exploration','Permitting','Charge drilling','Charge production pumps',...
    ...
    'CSP','TES','High-temp. power cycle','Low-temp power cycle'};
...
figure(2)
bar(LCOE_mat);
set(gca, 'XTick', 1:16, 'XTickLabel', xlab, 'TickLabelInterpreter', 'latex')
ylabel('LCOE, \$/kWh-e');
...
xlabel('Hour');
ylabel('Heat, MWh-th');
legend('Solar heat','Heat to HT-TES','Heat to HT power cycle', ...
