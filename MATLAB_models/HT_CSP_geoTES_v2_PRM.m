...
op_hour1 = [6:8 17:24]; % List of hours of operation
op_hour2 = [6:8 17:24]; % List of hours of operation
op_hour3 = [6:8 17:24]; % List of hours of operation
...
solar_multiple = 2.5;%5.2;
CSP_Tmax = 400 ; % Max temperature, C
CSP_Tmin = 250 ; % Min temperature, C
CSP_land_mult = 1.1 ;
...
HTPC_type = 'HE';
HTPC_design.Wout  = 100 ;% Power output, MW-e
HTPC_design.T0    = 25; % Design ambient temperature, C
HTPC_design.TIT   = -350 ; % Turbine inlet temperature, C. Calculate efficiency and Qin from this.
HTPC_design.eff   = 0.117 ;% Efficiency of the backpressure steam turbine section
HTPC_design.fan   = 0.0;
HTPC_design.Qin = -1; 
HTPC_foff = 'data\example_off_design.xlsx' ; % File location specifying off-design behaviour

HTPC_cost.power_block = 500 ; % power block, $/kW-e
HTPC_cost.HX = 500 ; % heat exchanger between TES and power block, $/kWh-th

% Set up high-temp power cycle class
HTPC = thermo_cycle_class(HTPC_type,HTPC_design,HTPC_foff,HTPC_cost, nY);
...
TES_cost.fluid = 1.5 ; % Storage fluid cost, $/kg
...
gTES_flowrate.P = 5.6 ; % Flow rate per production well, L/s
gTES_flowrate.I = 11 ; % Flow rate per injection well, L/s
gTES_Qmax = 1e5;%69.39;%920.63;%692;%46 ; % Maximum charging power input, MWh-th
...
HTPC.Tamb = repmat(out.Tamb,nY,1);
...
for i = 2:nY*8760
    ...
    if operate
        ...
        if CSP.power(i) > HTPC.Qin0
            ...
            % The low-temp power cycle also runs
            ...
            if dP * dT < TES.energy(i-1)
                ...
                % Calculate heat available in exhaust
                Qex = HTPC.Qin(i) - HTPC.Wout(i) ;

                % Heat required from GeoTES is
                dQ = LTPC.Qin0 - Qex ;
    ...
    % Otherwise all heat goes into TES until it is full, and then it goes
    % to HTPC and exhaust heat goes into the ground
    else
        % Send remaining heat to hot TES if there is capacity
        dP = CSP.power(i) ;
        dTES = TES.energy_capacity - TES.energy(i-1);
        if dTES > dP*dT
            ...
            % This goes to high-temp power cycle
            if dP > HTPC.Qin0
                HTPC.Qin(i) = HTPC.Qin0 ;
                HTPC = interpolate_off_design(HTPC,i);
                excess = dP - HTPC.Qin0;
            ...
            % Calculate heat available in exhaust and excess solar heat
...
discharge_air_fan = discharging_hours * LTPC.fan0 * (LTPC.Wout0+HTPC.Wout0) / 1e3 ; % Parasitic air cooler fan load
...
Ccap = [Total_Exploration_Cost FieldDevPlantPermitting  ...
    ...
    CSP.total_cost LTPC.total_cost HTPC.total_cost TES.total_cost]/1e6;
...
cap_fac = 100 * 1000 * net_elec / (HTPC.Wout0 + LTPC.Wout0 - Discharge_ProdPumping_Cost_Duty(1)/1e3 - Discharge_InjPumping_Cost_Duty(1)/1e3 - LTPC.fan0*LTPC.Wout0) / 8760 ;
...
legend('Solar heat','Heat to HT-TES','Heat to HT power cycle', ...
    ...
    'FontSize',10);
if save_figs == 1; save_fig(4, './Outputs/January_heat_flows',formats); end
...
legend('Solar heat','Heat to HT-TES','Heat to HT power cycle', ...
    ...
    'Orientation','vertical',...
    'FontSize',10);
if save_figs == 1; save_fig(5, './Outputs/August_heat_flows',formats); end
...
legend('Solar heat','Heat to HT-TES','Heat to HT power cycle', ...
    ...
    'Orientation','vertical',...
    'FontSize',10);
if save_figs == 1; save_fig(6, './Outputs/Feb_heat_flows',formats); end



figure(7)
n=24*27+(nY-1)*8760+1:24*31+(nY-1)*8760+1;
plot(HTPC.Wout(n)); hold on
plot(LTPC.Wout(n));
plot(HTPC.Wout(n)+LTPC.Wout(n)); hold off
pbaspect([3 1 1])
title('January 27')
xlabel('Hour');
ylabel('Power, MWh-e');
legend('HT power cycle','LT power cycle','Total', ...
    'Location','southoutside', ...
    'Orientation','vertical',...
    'FontSize',10);
if save_figs == 1; save_fig(7, './Outputs/January_power_flows',formats); end
...
if save_figs == 1; save_fig(8, './Outputs/August_power_flows',formats); end
...
if save_figs == 1; save_fig(9, './Outputs/Feb_power_flows',formats); end
