...
def build_operating_hours() -> tuple[tuple[int, int], tuple[int, int], tuple[int, int], np.ndarray, np.ndarray, np.ndarray]:
    ...
    op_hour1 = np.arange(17, 23)
    op_hour2 = np.arange(17, 23)
    op_hour3 = np.arange(17, 23)
...
def main() -> None:
    ...
    plt.xticks(np.arange(1, len(Ccap) + 1), [
        "Exploration",
        "Permitting",
        "Charge drilling",
    ...
    plt.xticks(np.arange(1, len(LCOE_mat) + 1), [
        "Exploration",
        "Permitting",
        "Charge drilling",
        "Charge production pumps",
    ...
    fig = plt.figure(3)
    hours = np.arange(*get_year_slice_indices(nY))
    plt.plot(hours, gTES.energy[hours] / 1000.0)
    plt.xlim([0, 8760])
    ...
    def doy_slice(year: int, start_month: int, start_day: int, end_month: int, end_day: int) -> slice:
        start = 24 * day_of_year(year, start_month, start_day)
        end = 24 * day_of_year(year, end_month, end_day)
        return slice(start, end)

    fig = plt.figure(4)
    n = doy_slice(2022, 1, 27, 1, 31)
