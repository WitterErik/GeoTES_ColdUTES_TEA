...
def load_electricity_prices(location: str, year: int) -> np.ndarray:
    if location == "Imperial CA":
        if loadmat is None:
            raise ImportError("scipy is required to load MAT price data for Imperial CA")
        mat_data = loadmat(PROJECT_ROOT / "data" / "caliPrices.mat")
        # MATLAB load stores struct arrays; preserve the 'cali.margCost' mapping.
        return np.ravel(mat_data["cali"]["margCost"])[0]

    if location == "Elk Hills CA":
        if year == 2020:
            raise ValueError("Elk Hills 2020 price data not available")
        file_name = f"202{year - 2020}_CAIOS_EK_DA.csv"
    ...
    if location == "ERCOT West":
        ...
        file_name = f"202{year - 2020}_PECO__DA.csv"
        return read_csv_column(PROJECT_ROOT / "data" / file_name, column_index=1, skip_rows=1)
...
def load_ambient_temperature(location: str) -> np.ndarray:
    if location == "Imperial CA":
        return read_csv_column(PROJECT_ROOT / "data" / "imperial_ca_32.835205_-115.572398_psmv3_60_tmy.csv", column_index=9, skip_rows=3)
    if location == "Elk Hills CA":
        return read_csv_column(PROJECT_ROOT / "data" / "Elk_Hills_tmy-2021.csv", column_index=11, skip_rows=3)
    if location == "ERCOT West":
        return read_csv_column(PROJECT_ROOT / "data" / "ERCOT_West_tmy-2021.csv", column_index=11, skip_rows=3)
...
def main() -> None:
    ...
    gCTES_Qmax = 10000.0
    gCTES_props = {"rho": 2000.0, "cp": 800.0, "void": 0.32, "depth": 500.0, "thickness": 100.0}
    gCTES_fluid = "water"
    ...
    gHTES_Qmax = 1000.0
    gHTES_props = {"rho": 2000.0, "cp": 800.0, "void": 0.32, "depth": 500.0, "thickness": 100.0}
    gHTES_fluid = "water"
    ...
    HE.Tamb = np.tile(Tamb, nY)  # MATLAB repmat(Tamb,nY,1) -> tile along first axis
    ...
    if gHTES.charge_injection.p < 20.0:
        gHTES.charge_injection.p = 20.0
    ...
    if gHTES.discharge_production.p < 20.0:
        gHTES.discharge_production.p = 20.0
        gHTES.discharge_production.calc_fluid_props("pT")

    gHTES.discharge_injection.T = gHTES.Tinit
    gHTES.discharge_injection.p = 20.0
    ...
    for i in range(1, n_hours):
        ...
        if elec_prices[i] <= econ.charge_price:
            ...
            # Cold storage discharge is intentionally omitted in this system, matching the original MATLAB logic.
    ...
    Ncold_source_well = gCTES.charge_prod_Nwell
    ...
    TotalNumberofWells = gCTES.charge_prod_Nwell
    ...
    Ncold_sink_well = gCTES.charge_inj_Nwell
    ...
    last_year_slice = slice((nY - 1) * hours, nY * hours)
    charging_hours = int(np.count_nonzero(HP.Win[last_year_slice] > 0))
    discharging_hours = int(np.count_nonzero(HE.Wout[last_year_slice] > 0))
    ...
    cold_discharge_production_pump = 0.0
    cold_discharge_injection_pump = 0.0
    ...
    econ.Eout = HE.Wout_tot - hot_discharge_injection_pump - hot_discharge_production_pump
    CB["RTeff_real"] = econ.Eout / econ.Ein if econ.Ein != 0 else np.nan

    econ.calc_fcr()
    econ.calc_revenue(HP.Win[last_year_slice], HE.Wout[last_year_slice])
    ...
    print(f"Average heat pump power input             = {np.mean(HP.Win[last_year_slice][HP.Win[last_year_slice] > 0]):6.2f} MW-e\n")
    ...
    print(f"Average power cycle output                = {np.mean(HE.Wout[last_year_slice][HE.Wout[last_year_slice] > 0]):6.2f} MW-e\n")
    print(f"Max. thermal power into hot storage       = {np.max(gHTES.power[last_year_slice]):6.2f} MW-th")
    print(f"Max. thermal power out of hot storage     = {-np.min(gHTES.power[last_year_slice]):6.2f} MW-th")
    print(f"Max. thermal power into cold storage      = {np.max(gCTES.power[last_year_slice]):6.2f} MW-th")
    print(f"Max. thermal power out of cold storage    = {-np.min(gCTES.power[last_year_slice]):6.2f} MW-th")
    print(f"Cold production pump power                = {Cold_source_ProdPumping_Cost_Duty[0] / 1e3:6.2f} MW-e")
    print(f"Cold injection pump power                 = {Cold_sink_InjPumping_Cost_Duty[0] / 1e3:6.2f} MW-e")
    print(f"Hot production pump power                 = {Hot_source_ProdPumping_Cost_Duty[0] / 1e3:6.2f} MW-e")
    print(f"Hot injection pump power                  = {Hot_sink_InjPumping_Cost_Duty[0] / 1e3:6.2f} MW-e")

    print("\nANNUAL ENERGY OUTPUTS\n")
    print(f"Number of charging hours                  = {charging_hours:6d} h")
    print(f"Number of discharging hours               = {discharging_hours:6d} h")
    ...
    print(f"Number of hot source wells      = {Nhot_source_well:6d}")
    print(f"Total hot source Drilling Cost  = {Total_Hot_source_Drilling_Cost / 1e6:6.2f} M$")
    print(f"Hot Source production pump cost = {Hot_source_ProdPumping_Cost_Duty[1] / 1e6:6.2f} M$")
    print(f"Hot Source injection pump cost  = {Hot_source_InjPumping_Cost_Duty[1] / 1e6:6.2f} M$")
    print(f"Hot Source flow line cost       = {Hot_source_FlowLineCost / 1e6:6.2f} M$\n")
    print(f"Number of hot sink wells        = {Nhot_sink_well:6d}")
    print(f"Total hot sink Drilling Cost    = {Total_Hot_sink_Drilling_Cost / 1e6:6.2f} M$")
    print(f"Hot Sink production pump cost   = {Hot_sink_ProdPumping_Cost_Duty[1] / 1e6:6.2f} M$")
    print(f"Hot Sink injection pump cost    = {Hot_sink_InjPumping_Cost_Duty[1] / 1e6:6.2f} M$")
    print(f"Hot Sink flow line cost         = {Hot_sink_FlowLineCost / 1e6:6.2f} M$\n")
    print(f"Number of cold source wells      = {Ncold_source_well:6d}")
    print(f"Total cold source Drilling Cost  = {Total_Cold_source_Drilling_Cost / 1e6:6.2f} M$")
    print(f"Cold Source production pump cost = {Cold_source_ProdPumping_Cost_Duty[1] / 1e6:6.2f} M$")
    print(f"Cold Source injection pump cost  = {Cold_sink_InjPumping_Cost_Duty[1] / 1e6:6.2f} M$")
    print(f"Cold Source flow line cost       = {Cold_source_FlowLineCost / 1e6:6.2f} M$\n")
    print(f"Number of cold sink wells        = {Ncold_sink_well:6d}")
    print(f"Total cold sink Drilling Cost    = {Total_Cold_sink_Drilling_Cost / 1e6:6.2f} M$")
    print(f"Cold Sink production pump cost   = {0.0:6.2f} M$")
    print(f"Cold Sink injection pump cost    = {Cold_sink_InjPumping_Cost_Duty[1] / 1e6:6.2f} M$")
    print(f"Cold Sink flow line cost         = {Cold_sink_FlowLineCost / 1e6:6.2f} M$\n")
